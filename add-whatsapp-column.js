...
async function addWhatsAppColumn() {
  ...
  try {
    ...
    if (tables.length === 0) {
      console.log('❌ Settings table tidak ditemukan.');
      return;
    }
    
    // Check if whatsapp_group_id column already exists
    const [columns] = await connection.execute(
      "SHOW COLUMNS FROM settings LIKE 'whatsapp_group_id'"
    );
    
    if (columns.length > 0) {
      console.log('✅ Kolom whatsapp_group_id sudah ada.');
    } else {
      // Add whatsapp_group_id column
      await connection.execute(
        'ALTER TABLE settings ADD COLUMN whatsapp_group_id VARCHAR(255) NULL'
      );
